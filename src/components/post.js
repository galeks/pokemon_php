...
import IMG from '../img/Pikachu-Pokemon.jpg';
            ...
            switch (type.name) {
                ...
                case "water":
                    return <a key={type.name} className="btn btn-m btn-primary"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
                case "bug":
                    return <a key={type.name} className="btn btn-m btn-danger"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
...
class Post extends Component {
    ...
    render() {
        ...
        return (
            <div className="card border border-dark rounded-1" key={info.pkdx_id} >
                <div className="card-body" id={"pokemon-" + info.pkdx_id}>
                    <img className="card-img-top border border-success rounded-1"
                         src={IMG}
                         alt="Card image cap"/>
